...
This skill is active during the `/work` phase of the development loop. It governs HOW implementation happens — every piece of new code goes through RED → GREEN → REFACTOR.

- **Input**: Planned tasks from feature-builder or `/plan`
- **Governs**: All implementation work during `/work`
...
- Critical user flows (login → create → view → edit → delete)
...
When building a feature with the feature-builder skill:

### 1. After Planning (Phase 1), Write Tests First (Phase 2)
